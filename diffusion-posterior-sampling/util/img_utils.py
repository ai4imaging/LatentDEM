...
class Blurkernel(nn.Module):
    def __init__(self, blur_type='gaussian', kernel_size=31, std=3.0, device=None):
        ...
        if kernel_size % 2==1:
            ...
            self.padding_module = nn.ReflectionPad2d(padding)
        
        else:
            padding = (self.kernel_size-1)//2
            self.padding_module = nn.ReflectionPad2d((padding + 1, padding, padding + 1, padding))
        self.conv = nn.Conv2d(3, 3, kernel_size, stride=1, padding=0, bias=False, groups=3)
        self.weights_init()
